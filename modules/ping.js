function ping(msg, ctx) {
    const pongMsgPool = ['Pong.',
        ...
        'PoNg.', 'pong', 'Pong', ':ping_pong:', ':ping_pong: Pong', 'whoops, i missed', 'no', 'PONG', 'gnoP', 'Pong³', '³', 'p³ng', '𝒫𝒪𝒩𝒢', '𝐏𝐎𝐍𝐆', 'what does that mean', 'poggers', '<:YeetAary:664279299255435267>', 'now you\'re thinking with portals', 'stop poking me', `Pong!, ${msg.member.id / 2}ms`, `Pong!, ${msg.member.id / 4}ms`, `Pong!, ${msg.member.id / 32}ms`, `Pong!, -${msg.member.id / 1.5}ms`, `Pong!, ${ctx.client.uptime}ms`, `Pong!, ${ctx.client.uptime}ms`,];
